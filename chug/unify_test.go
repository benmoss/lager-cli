...
var _ = Describe("Unify", func() {
	...
	It("should unify the independent streams", func(done Done) {
		...
		Expect(out).Should(gbytes.Say("non-lager-A"))
		Expect(out).Should(gbytes.Say("non-lager-B-1"))
		Expect(out).Should(gbytes.Say("A-1"))
		Expect(out).Should(gbytes.Say("B-1"))
		Expect(out).Should(gbytes.Say("A-2"))
		Expect(out).Should(gbytes.Say("B-2"))
		Expect(out).Should(gbytes.Say("B-3"))
		Expect(out).Should(gbytes.Say("non-lager-B-2"), "non-lager lines always beat lager lines")
		Expect(out).Should(gbytes.Say("A-3"))
		Expect(out).Should(gbytes.Say("B-4"))
	...
	Context("with a minimum time", func() {
		It("should only show log lines after that time", func(done Done) {
			...
			Expect(out).ShouldNot(gbytes.Say("non-lager-A"))
			Expect(out).ShouldNot(gbytes.Say("non-lager-B-1"))
			Expect(out).ShouldNot(gbytes.Say("A-1"))
			Expect(out).Should(gbytes.Say("B-1"))
			Expect(out).Should(gbytes.Say("A-2"))
			Expect(out).Should(gbytes.Say("B-2"))
			Expect(out).Should(gbytes.Say("B-3"))
	...
	Context("with a maximum time", func() {
		It("should only show log lines before that time", func(done Done) {
			...
			Expect(out).Should(gbytes.Say("B-3"))
			Expect(out).Should(gbytes.Say("non-lager-B-2"), "non-lager lines always beat lager lines")
			Expect(out).ShouldNot(gbytes.Say("A-3"))
			Expect(out).ShouldNot(gbytes.Say("B-4"))
	...
	Context("with time limits and filters", func() {
		It("should apply the time limits and the filters", func(done Done) {
			...
			Expect(out).ShouldNot(gbytes.Say("A-1"))
			Expect(out).Should(gbytes.Say("B-1"))
			Expect(out).ShouldNot(gbytes.Say("A-2"))
			Expect(out).Should(gbytes.Say("B-2"))
			Expect(out).ShouldNot(gbytes.Say("B-3"))
			Expect(out).Should(gbytes.Say("non-lager-B-2"), "non-lager lines always beat lager lines")
			Expect(out).ShouldNot(gbytes.Say("A-3"))
			Expect(out).ShouldNot(gbytes.Say("B-4"))
